...
class Connect4 extends JPanel implements ActionListener, MouseListener, MouseMotionListener {
    ...
    static class Board {
        ...
        static {
            ...
            players = new Color[]{Color.BLUE, Color.BLACK};
            turn = 0;
        }
        ...
        public static void drop() {
            if (board[hoverX/widthUnit - 1][0] != Color.GRAY) return;
            new Thread(() -> {
                Color color = players[turn];
                int x = hoverX;
                int i;
                for (i = 0; i < board[x/widthUnit - 1].length && board[x/widthUnit - 1][i] == Color.GRAY; i++) {
                    board[x/widthUnit - 1][i] = color;
                    try { Thread.currentThread().sleep(200); } catch(Exception ignored) {}
                    board[x/widthUnit - 1][i] = Color.GRAY;
                    if (gameDone) return;
                }
                if (gameDone) return;
